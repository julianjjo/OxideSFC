...
- **PPU** — background modes 0–7, including Mode 7 with EXTBG; 8×8 and 16×16 tiles; hi-res modes 5/6 (real 512-dot sampling collapsed by dot-pair averaging); sprites with the hardware's per-scanline 32-sprite/34-tile limits, priority rotation, and STAT77 range/time-over flags; offset-per-tile (modes 2/4); windowing; mosaic; color math; direct color; VRAM read prefetch buffer and VMAIN address remapping; the two separate PPU1/PPU2 open-bus registers.
- **DMA / HDMA** — all 8 channels with real immediate-DMA and per-scanline HDMA execution, hardware setup/sync overheads, and per-byte machine ticking (NMI/IRQ/HDMA fire mid-transfer at their real positions).
